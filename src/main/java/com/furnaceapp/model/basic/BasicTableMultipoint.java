...
class BasicTableMultipoint {
	private String data_value;
	private int data_order, fbid;

	BasicTableMultipoint( String d, int o, int f ) {
		data_value = d;
		data_order = o;
		fbid = f;
	}
	...
	int save( Connection conn ) throws Exception {
		...
		try {
			ps = conn.prepareStatement( "INSERT INTO furnacedata_multipoint( fbid, data_value, data_order ) " +
											" VALUES( ?,?,? )" );
			ps.setInt( 1, fbid );
